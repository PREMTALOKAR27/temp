...
const ReceiptManagement = () => {
    ...
    const fetchReceipts = async () => {
        try {
            const response = await axios.get(`/api/v1/bill-scan-ocr/all`);
    ...
    const handleVerify = async (id) => {
        try {
            await axios.put(`/api/v1/bill-scan-ocr/verify-receipt/${id}`);
        ...
        <Card sx={{  mb: 4, borderRadius: 3, boxShadow: 5 }}>
            <CardContent>
                ...
                <TableContainer component={Paper}>
                    <Table>
                        <TableHead sx={{ backgroundColor: "#3b4c68" }}>
                            <TableRow>
                        ...
                        <TableBody>
                            {receipts?.map((receipt) => (
                                <TableRow key={receipt.id}>
                                    ...
                                    <TableCell>
                                        <Button variant="contained"  color="success" size="small" onClick={()=> handleVerify(receipt.id)}>
                                            Verify
                                        </Button>
                                    </TableCell>
                                </TableRow>
